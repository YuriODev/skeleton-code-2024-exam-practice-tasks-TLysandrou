    ...
    def CheckforMatchWithPattern(self, Row, Column):
        for StartRow in range(Row + 2, Row - 1, -1):
            for StartColumn in range(Column - 2, Column + 1):
                try:
                    PatternList = []
                    PatternList += self.__GetCell(StartRow, StartColumn).GetSymbol()
                    PatternList += self.__GetCell(StartRow, StartColumn + 1).GetSymbol()
                    PatternList += self.__GetCell(StartRow, StartColumn + 2).GetSymbol()
                    PatternList += self.__GetCell(StartRow - 1, StartColumn + 2).GetSymbol()
                    PatternList += self.__GetCell(StartRow - 2, StartColumn + 2).GetSymbol()
                    PatternList += self.__GetCell(StartRow - 2, StartColumn + 1).GetSymbol()
                    PatternList += self.__GetCell(StartRow - 2, StartColumn).GetSymbol()
                    PatternList += self.__GetCell(StartRow - 1, StartColumn).GetSymbol()
                    PatternList += self.__GetCell(StartRow - 1, StartColumn + 1).GetSymbol()
                    PatternString = ''
                        ...
                        if P.MatchesPattern(PatternString, CurrentSymbol):
                            for C in PatternList:
                                if (C.GetSymbol() != '@' and not C.CheckSymbolAllowed(CurrentSymbol)):
                                    print("you cannot overlap same symbol")
                                    return 0
